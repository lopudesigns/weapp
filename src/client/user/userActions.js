...
export const followUser = username => (dispatch, getState, { authAPI }) => {
  ...
  return dispatch({
    type: FOLLOW_USER,
    payload: {
      promise: authAPI.follow(getAuthenticatedUserName(state), username),
    },
    meta: {
      username,
    },
...
export const unfollowUser = username => (dispatch, getState, { authAPI }) => {
  const state = getState();
  ...
  return dispatch({
    type: UNFOLLOW_USER,
    payload: {
      promise: authAPI.unfollow(getAuthenticatedUserName(state), username),
    },
    meta: {
      username,
    },
