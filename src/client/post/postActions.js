...
export const getContent = (author, permlink, afterLike) => (dispatch, getState, { client }) => {
  if (!author || !permlink) {
    return null;
  }
  ...
  return dispatch({
    ...
    payload: {
      promise: client.sendAsync('get_content', [author, permlink]).then(res => {
...
export const votePost = (postId, author, permlink, weight = 10000) => (
  ...
  return dispatch({
    type: LIKE_POST,
    payload: {
      promise: authAPI.vote(voter, post.author, post.permlink, weight).then(res => {
        if (window.analytics) {
