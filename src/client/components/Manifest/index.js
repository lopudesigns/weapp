...
import ManifestFormAux from './ManifestFormAux';
  ...
  sign = (auth) => {
    ...
    wehelpjs.broadcast.accountCreateWithDelegation(
      ...
      (err) => {
        ...
        if (err) {
					console.error(err)
					console.error(wehelpjs.operations.getErrorMessage(err) || intl.formatMessage({ id: 'general_error' }))		
          notification.error({
            message: intl.formatMessage({ id: 'error' }),
            description: wehelpjs.operations.getErrorMessage(err) || intl.formatMessage({ id: 'general_error' }),
          });
        } else {
          notification.success({
            message: intl.formatMessage({ id: 'success' }),
        ...
        <div className="Sign__content container text-left my-2 Sign__authorize">
          ...
          {step === 1 && options.customSignatory &&
            <div className="text-center">
              <ManifestFormAux roles={['owner','active']} sign={this.sign} />
            </div>
					}
					{step === 1 && !options.customSignatory &&
            <div className="text-center">
...
export default injectIntl(Manifest);
