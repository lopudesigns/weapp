...
export const local = (flags = 'i') =>
  new RegExp(urlSet({ domain: '(?:localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steem.weyoume.src)' }), flags);
export const remote = (flags = 'i') =>
  new RegExp(
    urlSet({ domain: `(?!localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steem.weyoume.src)${domainPath}` }),
    flags,
  );
