...
export const local = (flags = 'i') =>
  new RegExp(urlSet({ domain: '(?:localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steemit.weyoume.io)' }), flags);
export const remote = (flags = 'i') =>
  new RegExp(
    urlSet({ domain: `(?!localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steemit.weyoume.io)${domainPath}` }),
    flags,
  );
