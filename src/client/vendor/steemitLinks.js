...
export const local = (flags = 'i') =>
  new RegExp(urlSet({ domain: '(?:localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steem.weyoume.io)' }), flags);
export const remote = (flags = 'i') =>
  new RegExp(
    urlSet({ domain: `(?!localhost|(?:.*\\.)?steemit.com|(?:.*\\.)?busy.org|(?:.*\\.)?steem.weyoume.io)${domainPath}` }),
    flags,
  );
