...
export const getRate = () => (dispatch, getState, { client }) => {
  dispatch({
...
export const getRewardFund = () => (dispatch, getSelection, { client }) =>
  dispatch({
    type: GET_REWARD_FUND,
    payload: { promise: client.sendAsync('get_reward_fund', ['post']) },
  });

export const getTrendingTopics = () => (dispatch, getState, { client }) => {
  dispatch({
    type: GET_TRENDING_TOPICS,
    payload: {
      promise: client.sendAsync('get_trending_tags', [undefined, 50]).then(result =>
