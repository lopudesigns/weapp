...
module.exports = function createConfig(env = 'dev') {
  ...
  const config = {
    ...
    output: {
      ...
      publicPath: IS_DEV ? `https://alpha.weyoume.src:${CONTENT_PORT}/` : '/',
    },
    context: process.cwd(),
    plugins: [
      DEFINE_PLUGIN,
  ...
  if (IS_DEV) {
    config.entry = ['webpack-dev-server/client?https://alpha.weyoume.src:'+CONTENT_PORT, 'webpack/hot/dev-server', ...config.entry];
